...
y_pred = Y_hat_df['NHITS-median'].to_numpy().reshape(-1, 1)
...
output_df.to_csv(f'{results_dir}/nhits_dez2020_predictions.csv', index=False)
...
pd.DataFrame([metrics]).to_csv(f'{results_dir}/metrics_nhits.csv', index=False)


ax.set_title('Previsão com NHITS e Dados Reais', fontsize=22)
